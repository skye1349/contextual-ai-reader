...
请使用 macOS、Windows 或 Linux 桌面版 Obsidian。手机端可以阅读同步后的笔记和生成文件，但不能运行本地 CLI 与视频工具流程。从 Obsidian Community Plugins 安装的普通用户**不需要安装 Node.js，不需要 npm，也不需要下载源码仓库**。安装插件后，只需选择一种 AI 后端：

- **使用 Codex 账号/会员登录**：安装 [Codex App 或 Codex CLI](https://developers.openai.com/codex/cli)，CLI 模式运行 `codex login` 完成登录。`Codex command` 留空会自动检测，也可以填写完整路径。
- **使用 Claude Code 本地登录**：安装并登录 Claude Code，`Claude command` 留空自动检测，或填写完整路径。
- **使用 API token**：选择 OpenAI 或 Anthropic，然后填写对应 API key；不需要安装本地 AI CLI。

Markdown/PDF 选区、快速翻译、系统朗读、摘抄以及 API 后端本身不需要 YouTube 工具。只有受保护的 YouTube 字幕、纯视频帧截图、无 CC 转录需要同时安装 `yt-dlp` 和 `ffmpeg`：

| 系统 | 安装视频工具 | 默认检测位置 |
| --- | --- | --- |
| macOS | `brew install yt-dlp ffmpeg` | `/opt/homebrew/bin`、`/usr/local/bin` 或 PATH |
| Windows | `winget install yt-dlp.yt-dlp` 和 `winget install Gyan.FFmpeg` | PATH 中的 `yt-dlp.exe` / `ffmpeg.exe`，也支持 Scoop shims |
| Ubuntu/Debian | `sudo apt update && sudo apt install yt-dlp ffmpeg` | `/usr/bin` 或 PATH |
| 其他 Linux | 使用发行版的包管理器安装 `yt-dlp` 和 `ffmpeg` | `/usr/local/bin`、`~/.local/bin` 或 PATH |

安装命令行工具后请重启 Obsidian。如果自动检测失败，在插件设置中填写可执行文件的完整路径。无 CC 语音转文字还需要 Groq 或 OpenAI API key。

## 本地数据与缓存位置

插件设置、API key、生词缓存、YouTube 字幕和 YouTube 译文按 Vault 分别保存在：

```text
<你的-vault>/.obsidian/plugins/contextual-ai-reader/data.json
```

如果想保留缓存，不要误删 `data.json`、整个插件目录，不要重置插件数据，也不要在未复制该文件的情况下更换 Vault。每个 Vault 的缓存互相独立。YouTube 缓存保留最近使用的 30 个视频；字幕、语言和 Prompt 完全匹配时不会再次调用 AI。

截图 PNG 和生成的 transcript note 是普通 Vault 文件，位于设置中指定的文件夹；清除 `data.json` 缓存不会删除这些文件。

`data.json` 可能包含 API key，只能私密备份，不要提交到 Git、公开上传或分享。如果你会同步 `.obsidian` 文件夹，也要确保同步目标是可信的。
...
- `Translate selected text`：把选中文本替换为译文。
- `Insert translation below selected text`：保留原文，并把译文插到原文下方。
- `Read selected text aloud`：朗读选中文本。
- `Save selected text to excerpt note`：保存选中文本到摘抄文件。
...
在设置中配置 `Excerpt file`。点击 Book plus，或运行 `Save selected text to excerpt note`，插件会把内容追加到这个文件。
...
1. 在 Command Palette 中运行 `Open YouTube video`。
